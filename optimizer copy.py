...
WEIGHTS = {
                ...
                'n_comps': 8,      # Nombre de composants avant pénalité
                ...
                'resistors': 0.01    # Poids pour les résistances (favorise les solutions sans résistances)
            }
...
class CrossoverOptimizer:
    ...
    def fitness(self, individual, fixed_pols=None):
        ...
        for pols in pol_combinations:
            ...
            # =========================================================
            # --- NOUVEAU : Détection dynamique et adaptative ---
            # =========================================================
            ...
            for j in range(len(self.ways) - 1):
                ...
                # Fenêtres de recherche intelligentes selon le nombre de voies
                if len(self.ways) == 2:
                    # 2-voies : Un seul croisement (souvent entre 1000Hz et 3000Hz)
                    search_mask = (self.freqs > 800) & (self.freqs < 5000)
                
                else:
                    # 3-voies ou plus : On adapte selon la paire de HP évaluée
                    if j == 0:
                        # 1er croisement : Woofer vers Médium (Basses fréquences)
                        search_mask = (self.freqs > 150) & (self.freqs < 1000)
                    elif j == 1:
                        # 2ème croisement : Médium vers Tweeter (Hautes fréquences)
                        search_mask = (self.freqs > 1000) & (self.freqs < 8000)
                    else:
                        # Sécurité pour 4-voies et plus
                ...
                if np.any(search_mask):
                    ...
                    # On applique la pénalité (WEIGHTS['crossover']) sur +/- 1 octave autour de CE croisement spécifique
        ...
        # --- 3. CONTRAINTES LÉTALES ---
        penalty = 0.0
        
        # Pénalité si impédance d'entrée trop basse
    ...
    def run(self, generations=50, pop_size=60):
        ...
        if os.path.exists("00_best_crossover.json"):
            ...
            try:
               if len(self.ways) == 2: # NOUVEAU : Sécurité pour n'utiliser la graine qu'en 2-voies
        ...
        with ProcessPoolExecutor() as executor:
            ...
            for gen in range(generations):
                ...
                while len(new_pop) + len(raw_children) < pop_size:
                    # NOUVEAU : Sélection par Tournoi (meilleur que le random pur)
                    def tournament():
                    ...
                    # NOUVEAU : 30% de chance d'hybrider deux bons filtres
...
if __name__ == "__main__":
    ...
    config = [
        WayConfig("Woofer", r"crossovers\RS180-8+RST28A-4\RS180-8@0.frd", r"crossovers\RS180-8+RST28A-4\RS180-8.zma", target_type='LP', z_offset_m=0.06),
        WayConfig("Tweeter", r"crossovers\RS180-8+RST28A-4\RST28F-4@0.frd", r"crossovers\RS180-8+RST28A-4\RST28F-4.zma", target_type='HP', z_offset_m=0.0)
    ]
