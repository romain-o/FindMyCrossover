...
class TreeMutator:
    ...
    def _generate_random_component(self):
        ...
        comp_class = random.choice([Capacitor, Inductor])
        return comp_class()
    ...
    def _generate_lc_lowpass(self, target):
        """Génère une cellule passe-bas du 2ème ordre (Bobine série, Condensateur parallèle)."""
        l = Inductor(random.uniform(0.5e-3, 4.0e-3))
        c = Capacitor(random.uniform(2.2e-6, 33e-6))
        # Topologie : Signal -> Bobine -> (Condensateur vers la masse // Haut-Parleur)
        return SeriesNode(l, ParallelNode(c, target))

    def _generate_lc_highpass(self, target):
        """Génère une cellule passe-haut du 2ème ordre (Condensateur série, Bobine parallèle)."""
        c = Capacitor(random.uniform(1.0e-6, 15e-6))
        l = Inductor(random.uniform(0.1e-3, 1.5e-3))
        # Topologie : Signal -> Condensateur -> (Bobine vers la masse // Haut-Parleur)
        return SeriesNode(c, ParallelNode(l, target))
    ...
    def add_node(self, tree: Node) -> Node:
        ...
        if target_has_driver and choice < 0.50:
            sub_choice = random.random()
            
            if sub_choice < 0.20: 
                # Macro Zobel (Correction d'impédance)
                new_node = ParallelNode(target, self._generate_zobel())
                
            elif sub_choice < 0.40: 
                # Macro Notch (Correction de résonance)
                new_node = SeriesNode(self._generate_notch(), target)
                
            elif sub_choice < 0.60: 
                # Macro L-Pad (Atténuation globale)
                new_node = self._generate_lpad(target)
                
            elif sub_choice < 0.80:
                # NOUVEAU : Macro Passe-Bas (Coupe les aigus)
                new_node = self._generate_lc_lowpass(target)
                
            else:
                # NOUVEAU : Macro Passe-Haut (Coupe les graves)
                new_node = self._generate_lc_highpass(target)
